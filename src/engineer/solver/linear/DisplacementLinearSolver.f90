module DisplacementLinearSolver
    !! Subroutines to get displacements

    use iso_fortran_env, only: real64

    use EntityNodeSupport, only: NodeSupport

    use Structure, only: &
        node_supports, &
        Kg, Fg, Dg, &
        global_dimension, qtd_node_supports, qtd_dof_node

    implicit none
    private

    public :: calc_Dg
contains
    subroutine calc_Dg
        ! =========================================================================================
        ! Vars statement
        ! =========================================================================================
        ! Aux *************************************************************************************
        type(NodeSupport) :: node_support

        integer :: i  ! indices

        integer :: Dx_index
        integer :: Dy_index
        integer :: Rz_index

        real(real64), allocatable :: K_aux(:, :)
        real(real64), allocatable :: F_aux(:)

        ! External ********************************************************************************
        external :: dposv  ! solve symmetric positive defined matrix system (Lapack)

        integer :: info  ! status of operation (dposv - Lapack)

        ! =========================================================================================
        ! Initialization
        ! =========================================================================================
        ! Allocation
        allocate(K_aux(global_dimension, global_dimension))
        allocate(F_aux(global_dimension))

        ! Start values
        K_aux = Kg
        F_aux = Fg

        ! =========================================================================================
        ! Calculation
        ! =========================================================================================
        ! Add boundary ****************************************************************************
        do i = 1, qtd_node_supports
            node_support = node_supports(i)

            if (node_support%Dx) then
                Dx_index = (qtd_dof_node * (node_support%node - 1)) + 1
                K_aux(Dx_index, :) = 0d0
                K_aux(:, Dx_index) = 0d0
                K_aux(Dx_index, Dx_index) = 1d0
                F_aux(Dx_index) = 0d0
            end if

            if (node_support%Dy) then
                Dy_index = (qtd_dof_node * (node_support%node - 1)) + 2
                K_aux(Dy_index, :) = 0d0
                K_aux(:, Dy_index) = 0d0
                K_aux(Dy_index, Dy_index) = 1d0
                F_aux(Dy_index) = 0d0
            end if

            if (node_support%Rz) then
                Rz_index = (qtd_dof_node * (node_support%node - 1)) + 3
                K_aux(Rz_index, :) = 0d0
                K_aux(:, Rz_index) = 0d0
                K_aux(Rz_index, Rz_index) = 1d0
                F_aux(Rz_index) = 0d0
            end if
        end do

        ! Solution the system *********************************************************************
        Dg = F_aux
        call dposv('U', global_dimension, 1, K_Aux, global_dimension, Dg, global_dimension, info)

        if (info /= 0) error stop 'DPOSV - calc_Dg - Displacements: solution system.'

        ! Sum the prescribed displacement *********************************************************
        do i = 1, qtd_node_supports
            node_support = node_supports(i)

            if (node_support%Dx) then
                Dx_index = (qtd_dof_node * (node_support%node - 1)) + 1
                Dg(Dx_index) = node_support%Dx_value
            end if

            if (node_support%Dy) then
                Dy_index = (qtd_dof_node * (node_support%node - 1)) + 2
                Dg(Dy_index) = node_support%Dy_value
            end if

            if (node_support%Rz) then
                Rz_index = (qtd_dof_node * (node_support%node - 1)) + 3
                Dg(Rz_index) = node_support%Rz_value
            end if
        end do
    end subroutine calc_Dg
end module
